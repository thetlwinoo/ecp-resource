...
describe('People e2e test', () => {
  ...
  it('should create and save People', async () => {
    ...
    expect(await peopleUpdatePage.getPreferredNameInput()).to.eq(
      'preferredName',
      'Expected PreferredName value to be equals to preferredName'
    );
    ...
    if (await selectedIsPermittedToLogon.isSelected()) {
      ...
      expect(await peopleUpdatePage.getIsPermittedToLogonInput().isSelected(), 'Expected isPermittedToLogon not to be selected').to.be
        .false;
    } else {
      await peopleUpdatePage.getIsPermittedToLogonInput().click();
    ...
    if (await selectedIsExternalLogonProvider.isSelected()) {
      ...
      expect(await peopleUpdatePage.getIsExternalLogonProviderInput().isSelected(), 'Expected isExternalLogonProvider not to be selected')
        .to.be.false;
    } else {
      await peopleUpdatePage.getIsExternalLogonProviderInput().click();
      expect(await peopleUpdatePage.getIsExternalLogonProviderInput().isSelected(), 'Expected isExternalLogonProvider to be selected').to.be
        .true;
    }
    ...
    expect(await peopleUpdatePage.getUserPreferencesInput()).to.eq(
      'userPreferences',
      'Expected UserPreferences value to be equals to userPreferences'
    );
    ...
    expect(await peopleUpdatePage.getOtherLanguagesInput()).to.eq(
      'otherLanguages',
      'Expected OtherLanguages value to be equals to otherLanguages'
    );
    expect(await peopleUpdatePage.getValidFromInput()).to.eq('2000-12-31', 'Expected validFrom value to be equals to 2000-12-31');
    expect(await peopleUpdatePage.getValidToInput()).to.eq('2000-12-31', 'Expected validTo value to be equals to 2000-12-31');
